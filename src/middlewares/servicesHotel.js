...
const {createServiceH, getServiceHname, getServiceHById, updateServiceH, deleteServiceH} = require('../controllers/servicesHotel')
...
router.get('/:id', async (req, res, next)=>{
    const {id} = req.params
    try {
        return res.json(await getServiceHById(id))
    } catch (error) {
        next(error)
    }
})
